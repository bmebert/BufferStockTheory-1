...
\subsubsection{Existence and Continuity of the Ratio}% of $\TargetNrm{\mRat}_{t+1}/\mRat_{t}$.}
...
\begin{equation}\begin{split}
  \BalGroFac{\mFunc}_{t+1}(\aRat) % = \Ex_{t}[\BalGroFac{\aRat}\Rnorm+\tShk_{t+1}\pShk_{t+1} ] \\
   & = 1+\aRat\Rnorm
\\  \TargetNrm{\mFunc}_{t+1}(\aRat)% & = \Ex_{t}[\TargetNrm{\aRat}(\Rnorm/\pShk_{t+1})+\tShk_{t+1}\pShk_{t+1} ] \\
...
%  \\ & = \left({\aRat}(\mTrgNrm) \InvEpShkInv^{-1} - {\aRat}(\mTrgNrm)-(\mBalLvl-\mTrgNrm){\aRat}^{\prime} \right)\Rnorm
%  \\ & = \left({\aRat}(\mTrgNrm) \InvEpShkInv^{-1} - {\aRat}(\mTrgNrm)+(\mTrgNrm-\mBalLvl){\aRat}^{\prime} \right)\Rnorm
...
\begin{equation}\begin{split}
%    \log \Ex_{t}[\aRat(\BalGroRte{\mRat}_{t})\Rnorm+\pShk_{t+1}\tShk_{t+1}] & <     \log \left(\aRat(\BalGroRte{\mRat}_{t})\Rnorm+1\right) \\
...
Due to the model's nonlinearities the values of $\mRat$ at which the expected growth rate of $\cLevBF$ matches $\PGro$ is very slightly different from the $\mRat$ at which the growth rate at which expected growth of $\mLevBF$ is $\PGro$.  Defining $\grave{\mRat}$ as the $\mRat$ at which $\Ex_{t}[\cRatBF_{t+1}/\cRatBF_{t}]=\PGro$, we can show that to first order $\grave{\mRat} \approx \BalGroFac{\mRat}.$
\begin{align*}
...
\end{align*}
but at $\mRat=\BalGroFac{\mRat}$, $\Ex_{t}[\mRat_{t+1}]=\TargetNrm{\mRat}\approx\grave{\mRat}$.
\end{comment}
