...
\begin{align*}
  \bar{\Rnorm}\left(1-\cFunc^{\prime}({\mNrm}_t)\right) - 1 & <  \bar{\Rnorm}(1-\underbrace{(1-\RPFac)}_{\MinMPC}) - 1  \\
                                                            & = \bar{\Rnorm}\RPFac - 1 \\
...
\begin{align}
  \bar{\Rnorm} \equiv \Ex_{t}\left[\frac{\Rfree}{\PermGroFac \PermShk}\right] & < 1  \label{eq:RbarBelowOne}.
\end{align}
...
\textbf{If {\RIC} holds}. Equation~\eqref{eq:MinMPCDef} indicates that if the {\RIC} holds, then $\MinMPC >0$.  We show at the bottom of Section~\ref{sec:WRIC} that if the {\RIC} holds then $0 < \MinMPC < \cFunc^{\prime}(\mNrm_{t}) < 1$ so that 
\begin{align*}
  \Rnorm\left(1-\cFunc^{\prime}({\mNrm}_t)\right) - 1 & <  \Rnorm(1-\underbrace{(1-\RPFac)}_{\MinMPC}) - 1  \\
...
\begin{align}
  \Rnorm \equiv (\Rfree/\PermGroFac) & < 1  \label{eq:FHWCFails}.
\end{align}
...
\\ & = \log \PermGroFac (\aNrm_{t}\Rnorm+\permShk_{t+1}\TranShk_{t+1})- \log \mNrm_{t} 
\\ & = \log \PermGroFac (\aNrm_{t}\Rnorm+1+(\permShk_{t+1}\TranShk_{t+1}-1))- \log \mNrm_{t}
\end{align*}
...
\begin{equation}\begin{split}
  \BalGroFac{\mFunc}_{t+1}(\aNrm) % = \Ex_{t}[\BalGroFac{\aNrm}\Rnorm+\TranShk_{t+1}\PermShk_{t+1} ] \\
   & = 1+\aNrm\Rnorm
\\  \TargetNrm{\mFunc}_{t+1}(\aNrm)% & = \Ex_{t}[\TargetNrm{\aNrm}(\Rnorm/\PermShk_{t+1})+\TranShk_{t+1}\PermShk_{t+1} ] \\
 & = 1+\aNrm\underbrace{\Rnorm/\InvEPermShkInv}_{\bar{\Rnorm} > \Rnorm}
...
%  \\  \mTrgNrm - \mBalLvl & =1+ (\mTrgNrm-\cFunc(\mTrgNrm))\Rnorm/\InvEPermShkInv - (1+(\mBalLvl-\cFunc(\mBalLvl))\Rnorm)
 & = \left(\TargetNrm{\aNrm} \InvEPermShkInv^{-1} - \BalGroFac{\aNrm} \right)\Rnorm 
\\ & = \left({\aNrm}(\mTrgNrm) \InvEPermShkInv^{-1} - {\aNrm}(\mBalLvl) \right)\Rnorm 
\\ & = \left({\aNrm}(\mTrgNrm) \InvEPermShkInv^{-1} - \left({\aNrm}(\mTrgNrm+\mBalLvl-\mTrgNrm)\right) \right)\Rnorm 
\\ & \approx \left({\aNrm}(\mTrgNrm) \InvEPermShkInv^{-1} - \left({\aNrm}(\mTrgNrm)+(\mBalLvl-\mTrgNrm)\aFunc^{\prime}(\TargetNrm{\mNrm})\right) \right)\Rnorm 
%  \\ & = \left({\aNrm}(\mTrgNrm) \InvEPermShkInv^{-1} - {\aNrm}(\mTrgNrm)-(\mBalLvl-\mTrgNrm){\aNrm}^{\prime} \right)\Rnorm
%  \\ & = \left({\aNrm}(\mTrgNrm) \InvEPermShkInv^{-1} - {\aNrm}(\mTrgNrm)+(\mTrgNrm-\mBalLvl){\aNrm}^{\prime} \right)\Rnorm
  \\ (\mTrgNrm - \mBalLvl)(1-\underbrace{{\aNrm}^{\prime}(\mTrgNrm)\Rnorm}_{< \GPFacRaw < 1}) & = (\InvEPermShkInv^{-1} - 1)\TargetNrm{\aNrm}\Rnorm
%\\ & = \left((\mTrgNrm-\cFunc(\mTrgNrm)) \InvEPermShkInv^{-1} - (\mBalLvl-\cFunc(\mTrgNrm+\mBalLvl-\mTrgNrm)) \right)\Rnorm 
%\\ & = \left((\mTrgNrm-\cFunc(\mTrgNrm)) \InvEPermShkInv^{-1} -( (\mBalLvl-\cFunc(\mTrgNrm)+(\mBalLvl-\mTrgNrm)\MPC)) \right)\Rnorm 
\end{split}\end{equation}
The RHS of this equation is strictly positive because $\InvEPermShkInv^{-1}>1$ and both $\TargetNrm{\aNrm}$ and $\Rnorm$ are positive; while on the LHS, $(1-\Rnorm\aNrm^{\prime})>0$.  So the equation can only hold if $\TargetNrm{\mNrm}-\BalGroFac{\mNrm} > 0$.  That is, the target ratio exceeds the pseudo-steady-state ratio.\footnote{The use of the first order Taylor approximation could be substituted, cumbersomely, with the average of $\aFunc^{\prime}$ over the interval to remove the approximation in the derivations above.}
...
\\      \Ex_{t}[\log \left(\aNrm(\mNrm_{t})\Rnorm+\PermShk_{t+1}\TranShk_{t+1}\right)] & = \log \mNrm_{t}
\end{split}\end{equation}
...
\begin{equation}\begin{split}
    \exp(\Ex_{t}[\log \left(\aNrm(\BalGroRte{\mNrm})\Rnorm+\PermShk_{t+1}\TranShk_{t+1}\right)]) - (\aNrm(\BalGroFac{\mNrm})\Rnorm+1 ) = \BalGroRte{\mNrm} - \BalGroFac{\mNrm}
\end{split}\end{equation}

Now we use the fact that the expectation of the log is less than the log of the expectation, 
\begin{equation}\begin{split}
%    \log \Ex_{t}[\aNrm(\BalGroRte{\mNrm}_{t})\Rnorm+\PermShk_{t+1}\TranShk_{t+1}] & <     \log \left(\aNrm(\BalGroRte{\mNrm}_{t})\Rnorm+1\right) \\
 \exp(\Ex_{t}[\log \left(\aNrm(\BalGroRte{\mNrm})\Rnorm+\PermShk_{t+1}\TranShk_{t+1}\right)]) & < \left(\aNrm(\BalGroRte{\mNrm})\Rnorm+1    \right)
\end{split}\end{equation}
so
\begin{equation}\begin{split}
...
\\     \underbrace{\bar{\aFunc}^{\prime}\Rnorm}_{< \GPFacRaw} & < 1
\end{split}\end{equation}
