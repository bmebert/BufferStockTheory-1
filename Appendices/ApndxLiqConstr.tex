...
$1 < {(\Rfree\Discount)}^{1/\CRRA}/\PermGroFac$ implies that spending
...
resources to the next period at return $\Rfree$:\footnote{The point at
  ...
  \Discount \uFunc^{\prime}(\PermGroFac)$ which is $\cNrm_{\#} =
  \PermGroFac/{(\Rfree \Discount)}^{1/\CRRA}$ and the consumption function
  will be defined by
  $\cnstr{\cFunc}(\mNrm)=\min[\mNrm,\cNrm_{\#}+(\mNrm-\cNrm_{\#})\MinMPC
  ]$.}
\begin{equation}\begin{gathered}\begin{aligned}
  1  & < {(\Rfree \Discount)}^{1/\CRRA}\PermGroFac^{-1}    \notag
  \\ 1  & < \Rfree \Discount \PermGroFac^{-\CRRA} \notag
  \\  \uFunc^{\prime}(1)  & < \Rfree \Discount \uFunc^{\prime}(\PermGroFac)   \label{eq:EulerGICRawFailsEnd}.
...
\begin{equation}\begin{gathered}\begin{aligned}
  \mNrm_{\#}  & = (\mNrm_{\#}-1+\hNrm)\MinMPC
  \\ \mNrm_{\#}(1-\MinMPC)  & = (\hNrm - 1)\MinMPC
  \\ \mNrm_{\#}  & = (\hNrm - 1)\left(\frac{\MinMPC}{1-\MinMPC}\right)
\end{aligned}\end{gathered}\end{equation}
...
\begin{equation}\begin{gathered}\begin{aligned}
  \cNrm_{\#}^{n}  & = \GPFacRaw^{-n} \cNrm_{t} = \GPFac^{-n}. \label{eq:cPreHist}
\end{aligned}\end{gathered}\end{equation}
...
\begin{equation}\begin{gathered}\begin{aligned}
  ...
  % \\   \mathbb{C}_{t-n}^{t}  & = \cNrm_{t-n}(1+\GPFacRaw/\Rnorm+ \cdots + (\GPFac/\Rnorm)^{n}) \notag
...
$\bNrm_{t}=0$:
\begin{equation}\begin{gathered}\begin{aligned}
  \bNrm_{\#}^{n} & =  \mathbb{C}_{t-n}^{t} - \overbrace{\left(\frac{1-\Rnorm^{-(n+1)}}{1-\Rnorm^{-1}}\right)}^{\hNrm_{\#}^{n}} \label{eq:bPound}.
\end{aligned}\end{gathered}\end{equation}
...
\begin{eqnarray*}
  \lim_{n \rightarrow \infty} \left(\frac{\GPFacRaw^{-n}-{(\RPFac/\GPFac)}^{n}\RPFac}{1-\RPFac}\right)  & = \infty
  \\  \lim_{n \rightarrow \infty} \left(\frac{\GPFacRaw^{-n}-\Rnorm^{-n}\RPFac}{1-\RPFac}\right)  & = \infty
...
\begin{equation}\begin{gathered}\begin{aligned}
  % \lim_{n \rightarrow \infty} \left(\frac{\GPFacRaw^{-n}-\Rnorm^{-n}\RPFac}{1-\RPFac}\right) + \left(\frac{1-\Rnorm^{-(n+1)}}{\Rnorm^{-1}-1}\right)  & = \infty \\
  \lim_{n \rightarrow \infty} \left(\frac{\Rnorm^{-n}\RPFac-\GPFacRaw^{-n}}{\RPFac-1}\right) + \left(\frac{1-\Rnorm^{-(n+1)}}{\Rnorm^{-1}-1}\right)  & = \infty \notag
  \\   \lim_{n \rightarrow \infty} \left(\frac{\RPFac}{\RPFac-1}-\frac{\Rnorm^{-1}}{\Rnorm^{-1}-1}\right)\Rnorm^{-n}-\left(\frac{\GPFacRaw^{-n}}{\RPFac-1}\right)  & = \infty \label{eq:FHWCfails} 
  % \\   \lim_{n \rightarrow \infty} \left(\frac{\RPFac(\Rnorm^{-1}-1)}{(\Rnorm^{-1}-1)(\RPFac-1)}-\frac{\Rnorm^{-1}(\Rnorm^{-1}-1)}{(\Rnorm^{-1}-1)(\RPFac-1)}\right)\Rnorm^{-n}-\left(\frac{\GPFacRaw^{-n}}{\RPFac-1}\right)  & = \infty. 
\end{aligned}\end{gathered}\end{equation}
...
\begin{eqnarray*}
  \lim_{n \rightarrow \infty} \left(\frac{\GPFacRaw^{-n}}{1-\RPFac}\right)-\Rnorm^{-n}\left(\frac{\RPFac}{1-\RPFac}+\frac{\Rnorm^{-1}}{\Rnorm^{-1}-1}\right)  & = \infty
\end{eqnarray*}
...
which with a bit of algebra\footnote{
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
    \left(\frac{\GPFacRaw^{-n}}{\GPFac^{-n}/(1-\RPFac) - (1-\Rnorm^{-1}\Rnorm^{-n})/(1-\Rnorm^{-1})}\right)  & = \left(\frac{1}{1/(1-\RPFac) + \Rnorm^{-n}\Rnorm^{-1}/(1-\Rnorm^{-1})}\right)
...
\begin{eqnarray}
  \lim_{n \rightarrow \infty} \left(\frac{\RPFac(\Rnorm^{-1}-1)}{(\Rnorm^{-1}-1)(\RPFac-1)}-\frac{\Rnorm^{-1}(\RPFac-1)}{(\Rnorm^{-1}-1)(\RPFac-1)}\right)\Rnorm^{-n}-\left(\frac{\GPFacRaw^{-n}}{\RPFac-1}\right)  & = \infty.  
\end{eqnarray}
which makes clear that with $\cncl{\FHWC} \Rightarrow \Rnorm^{-1} > 1$ and $\cncl{\RIC} \Rightarrow \RPFac > 1$ the numerators and denominators of both terms multiplying $\Rnorm^{-n}$ can be seen transparently to be positive.  So, the terms multiplying
$\Rnorm^{-n}$ in~\eqref{\localorexternallabel{eq:FHWCfails}} will be positive if
\begin{eqnarray*}
  \RPFac \Rnorm^{-1} - \RPFac  & > & \Rnorm^{-1}\RPFac-\Rnorm^{-1}
  \\ \Rnorm^{-1}  & > & \RPFac
  \\ \PermGroFac  & > & \APFac
\end{eqnarray*}
...
\end{aligned}\end{gathered}\end{equation}
but that limit is approached gradually, starting from a positive value, and consequently the consumption function is \textit{not} the degenerate $\cnstr{\cFunc}(\mNrm)=0$.  (Figure~\ref{fig:PFGICHoldsFHWCFailsRICFails} presents an example for $\CRRA=2$, $\Rfree=0.98$, $\Discount = 1.00$, $\PermGroFac = 0.99$; note that the horizontal axis is bank balances $\bNrm = \mNrm-1$; the part of the consumption function below the depicted points is uninteresting --- $\cNrm = \mNrm$ --- so not worth plotting).
...
\begin{comment}
  ...
  \begin{eqnarray*}
    \mathbb{C}_{t-n}^{t}  & =  \left(\frac{\GPFacRaw^{-n}-\GPFac^{-n}\RPFac^{n+1}}{1-\RPFac}\right)
    ...
    \\ \Rnorm^{n}\mathbb{C}_{t-n}^{t}   & =  \left(\frac{(\Rfree/\APFac)^{n}-\RPFac}{1-\RPFac}\right)
  \end{eqnarray*}
  but \cncl{\RIC} implies that $\APFac > \Rfree$ so
  \begin{eqnarray*}
    \lim_{n \rightarrow \infty} \Rnorm^{n}\mathbb{C}_{t-n}^{t}   & =  \left(\frac{-\RPFac}{1-\RPFac}\right)
  \end{eqnarray*}
  which means that in the limit
  \begin{equation}\begin{gathered}\begin{aligned}
    \lim_{n \rightarrow \infty} \Rnorm^{n}\bNrm_{\#}^{n}  & =  \left(\frac{-\RPFac}{1-\RPFac}\right) - \Rnorm^{n}\left(\frac{1-\Rnorm^{-1}\Rnorm^{-n}}{1-\Rnorm^{-1}}\right) %\label{eq:bPoundLim}
    \\  & = \left(\frac{-\RPFac}{1-\RPFac} + \frac{-\Rnorm^{-1}}{1-\Rnorm^{-1}}\right) - \left(\frac{\Rnorm^{n}}{1-\Rnorm^{-1}}\right)
  \end{aligned}\end{gathered}\end{equation}
  so we can solve for the limiting $n$ as a function of $\bNrm$ via
  \begin{equation}\begin{gathered}\begin{aligned}
    \bNrm + \left(\frac{1}{1-\Rnorm^{-1}}\right)  & = \Rnorm^{-\hat{n}}\left(\frac{-\RPFac}{1-\RPFac} + \frac{-\Rnorm^{-1}}{1-\Rnorm^{-1}}\right)
    \\ \log \left(\bNrm + \left(\frac{1}{1-\Rnorm^{-1}}\right)\right)  & = -\hat{n} \log \Rnorm + \log \left(\frac{-\RPFac}{1-\RPFac} + \frac{-\Rnorm^{-1}}{1-\Rnorm^{-1}}\right)
    \\ \frac{\log \left(\frac{-\RPFac}{1-\RPFac} + \frac{-\Rnorm^{-1}}{1-\Rnorm^{-1}}\right)-\log \left(\bNrm + \left(\frac{1}{1-\Rnorm^{-1}}\right)\right)}{\log \Rnorm }  & = \hat{n}
  \end{aligned}\end{gathered}\end{equation}
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
    \nabla(n) \equiv   \lim_{n \rightarrow \infty}\Rnorm^{n}\mathbb{C}_{t-n}^{t}-\Rnorm^{n}\mathbb{C}_{t-n}^{t}  & =  \left(\frac{\RPFac^{-n}}{1-\RPFac}\right).
  \end{aligned}\end{gathered}\end{equation}
