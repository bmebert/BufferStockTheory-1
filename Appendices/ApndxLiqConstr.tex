...
Under perfect foresight in the presence of a liquidity constraint requiring $\bRat \geq 0$, this appendix taxonomizes the varieties of the limiting consumption function $\cnstr{\cFunc}(\mRat)$ that arise under various parametric conditions.  Results are summarized in table~\ref{table:LiqConstrScenarios}.
...
resources to the next period at return $\Rfree$:\footnote{The point at
  ...
  will be defined by
  $\cnstr{\cFunc}(\mRat)=\min[\mRat,\cRat_{\#}+(\mRat-\cRat_{\#})\MinMPC
  ]$.}
...
\begin{equation}\begin{gathered}\begin{aligned}
  \cnstr{\cFunc}_{T-n}(\mRat)  & = 0 (\bRat_{t}+\hRat).
\end{aligned}\end{gathered}\end{equation}
...
$\cnstr{\cFunc}_{T-n}(\mRat) = 0$ or
$\cnstr{\cFunc}_{T-n}(\mRat) = \infty$ depending on the relative
...
$\infty$.\footnote{The knife-edge case is where $\Pat = \PGro$, in
  ...
  function is $\cnstr{\cFunc}(\mRat)=\min[\mRat,1]$.}
...
is obliged to choose $\cnstr{\cFunc}(\mRat) = \mRat$.\footnote{As an
...
$\cnstr{\cFunc}(\mRat)$ defined by linearly connecting the points
$\{\mRat_{\#}^{n},\cRat_{\#}^{n}\}$ for integer values of $n \geq 0$
(and setting $\cnstr{\cFunc}(\mRat)=\mRat$ for $\mRat<1$).  This
...
\begin{equation}\begin{gathered}\begin{aligned}
  \lim_{m \rightarrow \infty} \cnstr{\pmb{\MPC}}(\mRat)  & = 1-\PatR.
\end{aligned}\end{gathered}\end{equation}
...
\begin{equation}\begin{gathered}\begin{aligned}
  \lim_{\mRat \rightarrow \infty} \cnstr{\pmb{\MPC}}(\mRat)   & = 0,
\end{aligned}\end{gathered}\end{equation}
but that limit is approached gradually, starting from a positive value, and consequently the consumption function is \textit{not} the degenerate $\cnstr{\cFunc}(\mRat)=0$.  (Figure~\ref{fig:PFGICHoldsFHWCFailsRICFails} presents an example for $\CRRA=2$, $\Rfree=0.98$, $\Discount = 1.00$, $\PGro = 0.99$; note that the horizontal axis is bank balances $\bRat = \mRat-1$; the part of the consumption function below the depicted points is uninteresting --- $\cRat = \mRat$ --- so not worth plotting).
