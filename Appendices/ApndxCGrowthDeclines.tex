...
define
\begin{align*}
  \pmb{\Upsilon}(\mNrm_{t})  & \equiv  \PermGroFac_{t+1} \usual{\cFunc}(\Rnorm_{t+1}\aFunc(\mNrm_{t})+\TranShkAll_{t+1})/\usual{\cFunc}(\mNrm_{t})  = \cLvl_{t+1}/\cLvl_{t}
\end{align*}
...
\begin{align}
  \Ex_{t}\left[\PermGroFac_{t+1} \left(\frac{\usual{\cFunc}^{\prime}(\mNrm_{t+1})\Rnorm_{t+1}\aFunc^{\prime}(\mNrm_{t})\usual{\cFunc}(\mNrm_{t})-\usual{\cFunc}(\mNrm_{t+1})\usual{\cFunc}^{\prime}(\mNrm_{t})}{\usual{\cFunc}{(\mNrm_{t})}^{2}}\right)\right]  & < 0 \label{eq:kappaPrimeLT0}.
\end{align}
...
$\PermGroFac_{t+1}\Rnorm_{t+1}=\Rfree$, the portion of the LHS of equation~\eqref{eq:kappaPrimeLT0} in brackets can be manipulated to yield
