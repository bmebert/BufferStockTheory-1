...
\begin{equation}\begin{gathered}\begin{aligned}
 \eFunc_{t}{(\mNrm_{t})}^{-\CRRA}  & = \DiscFac \Rfree \Ex_{t}\left[{\left(\eFunc_{t+1}({\mNrm}_{t+1})\left(\frac{\overbrace{\Rfree \aFunc_{t}(\mNrm_{t})+{\PGro}_{t+1}{\tShkAll}_{t+1}}^{=\mNrm_{t+1}\PGro_{t+1}}}{\mNrm_{t}}\right)\right)}^{-\CRRA }\right] \label{eq:eFuncEuler}
\\  & = \phantom{ + }\pNotZero \DiscFac \Rfree \mNrm_{t}^{\CRRA} \Ex_{t}\left[ {\left(\eFunc_{t+1}(\mNrm_{t+1}) \mNrm_{t+1}\PGro_{t+1}\right)}^{-\CRRA} |~ \tShkAll_{t+1}>0 \right] \notag
\\ &  + \pZero  \DiscFac \Rfree^{1-\CRRA} \Ex_{t}\left[{\left(\eFunc_{t+1}(\mathcal{R}_{t+1}\aFunc_{t}(\mNrm_{t}))\frac{\mNrm_t-\cFunc_{t}(\mNrm_{t})}{\mNrm_{t}}\right)}^{-\CRRA} |~ \tShkAll_{t+1} = 0 \right]   .
...
\aFunc_{t}(\mNrm) = 0$,
$\Ex_{t}[{(\eFunc_{t+1}(\mNrm_{t+1})\mNrm_{t+1}\PGro_{t+1})}^{-\CRRA}~|~\tShkAll_{t+1}>0]$
...
\begin{eqnarray*}
 {({\mNrm_{t}}\eFunc_{t}(\mNrm_{t}))}^{-\CRRA}  & = \DiscFac \Rfree \Ex_{t}\left[{\left(\eFunc_{t+1}(\aFunc_{t}(\mNrm_{t})\Rnorm_{t+1})\left({\Rfree \aFunc_{t}(\mNrm_{t})}\right)\right)}^{-\CRRA}\right]
\end{eqnarray*}
and using L'H\^opital's rule $\lim_{\mNrm_{t} \rightarrow \infty} \eFunc_{t}(\mNrm_{t}) = \MinMPC_{t}$, and $\lim_{\mNrm_{t} \rightarrow \infty} \eFunc_{t+1}(\aFunc_{t}(\mNrm_{t})\Rnorm_{t+1}) = \MinMPC_{t+1}$ so a further limit of the Euler equation is\hypertarget{MPCnvrs}{}
\begin{eqnarray*}
...
\begin{eqnarray*}
  \cNrm_{t}\underbrace{\left(1 + \PatR + \PatR^{2} + \cdots \right)}_{=1+\PatR(1+\PatR\MinMPC_{t+2}^{-1})\ldots}   & = \cNrm_{t}\MinMPC_{T-n}^{-1}.
\end{eqnarray*}
