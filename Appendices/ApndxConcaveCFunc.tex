...
will hold if ${(\MaxMPC \mRat_{t})}^{1-\CRRA}$ is $\boundFunc$-bounded.  We use
the bounding function
\begin{equation}\begin{gathered}\begin{aligned}
  \label{eq:boundFunc}
  \boundFunc(\mRat)  & = \eta + \mRat^{1-\CRRA},
\end{aligned}\end{gathered}\end{equation}
...
\boundFunc(\mRat_{t})$. The proof will be more compact if we define
...
Using $\boundFunc(\mRat)= \eta + \mRat^{1-\CRRA}$
