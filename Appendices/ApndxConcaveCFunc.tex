...
\section{Existence of Concave \texorpdfstring{$\cFunc$}{c} Function}\label{sec:ApndxcExists}
...
[\MinMinMPC \mNrm_{t}, \MaxMPC \mNrm_{t}]
}\max \left\{
...
$\uFunc(\MaxMPC \mNrm_{t})$. Thus, condition (2)
will hold if ${(\MaxMPC \mNrm_{t})}^{1-\CRRA}$ is $\boundFunc$-bounded.  We use
...
$\{\TMap\mathbf{0}\}(\mNrm_{t})= \uFunc(\MaxMPC \mNrm_{t})$
is clearly
$\boundFunc$-bounded.
...
\begin{align*}
\DiscFac \Ex_{t}[{\PermGroFac}_{t+1}^{1-\CRRA}{(\hat{\aNrm}_{t}\Rnorm_{t+1}+\TranShkAll_{t+1})}^{1-\CRRA}]-\mNrm_{t}^{1-\CRRA}  & < \eta(1-\underbrace{\DiscFac\Ex_{t}{\PermGroFac}_{t+1}^{1-\CRRA}}_{=\DiscAlt})
\end{align*}
which by imposing \PFFVAC~(equation~\eqref{\localorexternallabel{eq:PFFVAC}}, which says $\DiscAlt<1$) can be rewritten as:
\begin{equation}\begin{gathered}\begin{aligned}
 \eta>\frac{\DiscFac \Ex_{t}\left[{\PermGroFac}_{t+1}^{1-\CRRA}{(\hat{\aNrm}_{t}\Rnorm_{t+1}+\TranShkAll_{t+1})}^{1-\CRRA}\right]-\mNrm_{t}^{1-\CRRA}}{1-\DiscAlt}\label{eq:KeyCondition}.
\end{aligned}\end{gathered}\end{equation}
...
\begin{equation}\begin{gathered}\begin{aligned}%
  \lefteqn{\pNotZero\DiscFac\Ex_{t}\left[{\PermGroFac}_{t+1}^{1-\CRRA}{(\hat{\aNrm}_{t}\Rnorm_{t+1}+\TranShkEmp_{t+1}/\pNotZero)}^{1-\CRRA}\right]
 +\pZero\DiscFac\Ex_{t}\left[{\PermGroFac}_{t+1}^{1-\CRRA}{(\hat{\aNrm}_{t}\Rnorm_{t+1})}^{1-\CRRA}\right]-\mNrm_{t}^{1-\CRRA}\notag~~~}  \\ 
 ~~~\leq & \pNotZero\DiscFac\Ex_{t}\left[{\PermGroFac}_{t+1}^{1-\CRRA}{((1-\MaxMPC)\mNrm_{t}\Rnorm_{t+1}+\TranShkEmp_{t+1}/\pNotZero)}^{1-\CRRA}\right]
 +\pZero\DiscFac\Rfree^{1-\CRRA}{((1-\MaxMPC)\mNrm_{t})}^{1-\CRRA}-\mNrm_{t}^{1-\CRRA}\notag\\
 ~~~= & \pNotZero\DiscFac\Ex_{t}\left[{\PermGroFac}_{t+1}^{1-\CRRA}{((1-\MaxMPC)\mNrm_{t}\Rnorm_{t+1}+\TranShkEmp_{t+1}/\pNotZero)}^{1-\CRRA}\right]
...
$\MinMinMPC \mNrm_{t} \leq c_{t} \leq \MaxMPC \mNrm_{t}$.  However,
in the discussion of the consumption function bounds, we
showed only (in~\eqref{\localorexternallabel{eq:cBounds}}) that $\MinMinMPC_{t} \mNrm_{t} \leq \cFunc_{t}(\mNrm_{t})
\leq \MaxMPC_{t} \mNrm_{t}$.  (The difference is in the presence
