% -*- mode: LaTeX; TeX-PDF-mode: t; -*- # Tell editor the file type

\newcommand{\texname}{BufferStockTheory}% Keyname for the paper
\input{./econtexRoot} % Set paths (like, \LaTeXInputs for resources)
\documentclass[titlepage, % Some doc do not need a title page 
headings=optiontotocandhead]{\econtex} % allow headers
...
\usepackage{\LaTeXInputs/\texname}% Styling for paper named \texname (and subfiles)
\usepackage{\LaTeXInputs/\texname-Shortcuts}% LaTeX Shortcuts for whole paper
\bibliographystyle{\econtexBibStyle}
\begin{document}

% Redefine \onlyinsubfile command from \texname.sty file:
...
\endinput % not needed here; useful if end of doc moves up (ignore rest of file)
