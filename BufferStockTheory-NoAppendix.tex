...
In a Harmenberg-invariant economy (and therefore also any Szeidl-invariant economy), a similar proposition holds in the cross-section as a direct implication of the fact that a constant proportion of aggregate permanent income is accounted for by the successive sets of consumers with any particular $\mRat$.  This fact is essentially Harmenberg's definition of the density of the permanent-income-invariant distribution of a variable (say, $\mRat$).\footnote{His $\tilde{\psi}^{m}$ has an invariant distribution.}  Call $\cLevBF_{t}(\mRat)$ the total amount of consumption at date $t$ by persons with market resources $\mRat$, and note that in the invariant economy it is given by the converged consumption function $\cFunc(\mRat)$ multiplied by the amount of permanent income accruing to such people $\pLevBF_{t}(\mRat)\PLev_{t}$.  But Harmenberg's proof shows that $\pLevBF(\mRat)$ is invariant,\footnote{In his notation, this is $\tilde{\psi}^{\mRat}$.} and aggregate permanent income grows according to $\PLev_{t+1} = \PGro \PLev_{t}$.  So 
\begin{equation*}
  \begin{split}
...
Harmenberg has a brief discussion demonstrating that the covariance between the individual consumption ratio $\cRat$ and the idiosyncratic component of permanent income level $\pLevBF$ does not shrink to zero over time; so covariances are another potential target of measurement for construction of microfoundations.  %Here we draw out some of those implications.

Consider again a date-0 Harmenberg-converged economy, and define the mean value of the consumption ratio as $\bar{\cRat}_{t+n} \equiv \Mean[\cRat_{t+n}]$.  Normalizing perid-0 aggregate permanent income to $\PLev_{0}=1$, total consumption at $t+1$ and $t+2$ are
\begin{equation}\label{eq:atp2vsatp1}
  ...
  \end{split}
\end{equation}
but Harmenberg's proof that $\CLevBF_{t+2}-\PGro \CLevBF_{t+1}=0$ allows us to multiply $\CLevBF_{t+1}$ by $\PGro$ and subtract from $\CLev_{t+2}$. Dropping the arguments to the $\cv$ terms for clarity, this yields:
\begin{align*}
...
In the special case of a Szeidl-invariant economy, $\bar{\cRat}_{t+2} = \bar{\cRat}_{t+1}$, implying that the economy exhibits balanced growth in the covariance:
\begin{align}
...
The more interesting case is when the economy is Harmenberg- but not Szeidl-invariant.  In that case, if the $\cv$ and the $\Mean[\cRat]$ terms have constant growth factors $\Grw_{\cov}$ and $\Grw_{\bar{\cRat}}$, an equation corresponding to \eqref{eq:atp2vsatp1} will hold in $t+n$:
