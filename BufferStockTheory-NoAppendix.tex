...
\renewcommand{\forcedate}{October 28, 2021}\date{\forcedate}
...
Consider a date-0 Harmenberg-converged economy, and define the mean value of the consumption ratio as $\cRatAvg_{t+n} \equiv \Mean[\cRat_{t+n}]$.  Normalizing period-0 aggregate permanent income to $\PLevBF_{0}=1$, total consumption at $t+1$ and $t+2$ are
...
The more interesting case is when the economy is Harmenberg- but not Szeidl-invariant.  In that case, if the $\cv$ and the $\cRatAvg$ terms have constant growth factors $\Grw_{\cov}$ and $\Grw_{\cRatAvg}$, an equation corresponding to \eqref{eq:cNrmvsCov} will hold in $t+n$:
