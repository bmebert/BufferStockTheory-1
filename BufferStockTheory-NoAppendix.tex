...
\renewcommand{\forcedate}{October 13, 2021}\date{\forcedate}
...
\cite{harmenbergInvariant} shows\footnote{The test of the Harmenberg method currently implemented in the Econ-ARK toolkit shows that, for the same computational resources, Harmenberg's method reduces the standard error of the simulated aggregate consumption series by a factor of more than 8; see the last part of \href{https://github.com/econ-ark/BufferStockTheory/blob/master/reproduce/computed.sh}{\texttt{reproduce/computed.sh}}.} that his reformulation of the problem can reduce costs of calculation enormously.  The method also appears to be promisingly generalizable: For any statistic $\mathcal{S}$ that is the key focus of an analysis, it should be possible to improve the efficiency of simulations aimed at calculating that statistic by constructing an $\mathcal{S}$-weighted distribution rather than a neutral distribution.
...
Consider again a Harmenberg-converged economy at date $t=0$, and define $\bar{\aRat}_{t+n} \equiv \Mean[\aRat_{t+n}]$.  Total assets at $t+1$ and $t+2$ are
\begin{align}
%  \mathbf{A}_{t}  % & l= \bar{\tilde{\aRat}}_{t} = \Mean[a_{t}\pLevBF_{t}] = \Mean[a_{t}]\Mean[\pLevBF_{t}] + \cv_{t}(\aRat_{t}, \pLevBF_{t}) \\
%                    & = %\bar{\tilde{\aRat}}_{t} =
%                      \Mean[a_{t}\pLevBF_{t}] = \Mean[a_{t}]\overbrace{\Mean[\pLevBF_{t}]}^{=1} + \cv_{t}(\aRat_{t}, \pLevBF_{t})  \\
  \mathbf{A}_{t+1} & = \Mean[a_{t+1}\pLevBF_{t+1}] = \bar{\aRat}_{t+1}\PGro^{1} + \cv_{t+1}(\aRat_{t+1}, \pLevBF_{t+1}) \notag
\\  \mathbf{A}_{t+2} & = \Mean[a_{t+2}\pLevBF_{t+2}] = \bar{a}_{t+2}\PGro^{2} + \cv_{t+2}(\aRat_{t+2}, \pLevBF_{t+2}) \label{eq:atp2vsatp1}
\end{align}
but Harmenberg's proof that $\mathbf{A}_{t+2}-\PGro \mathbf{A}_{t+1}=0$ allows us to multiply the first of these by $\PGro$ and subtract from the second, yielding (dropping the arguments to the $\cv$ terms for clarity):
\begin{align*}
%  \mathbf{A}_{t+1} & = \Mean_{t+1}[a_{t+1}\pLevBF_{t+1}] = \Mean_{t+1}[a_{t+1}\PGro\pShk_{t+1}\pLevBF_{t}] + \cv_{t+1}(\aRat_{t+1}, \pLevBF_{t}\PGro \pShk_{t+1})
%\\  & = \PGro \Harm{\Mean}_{t+1}[a_{t+1}]\underbrace{\Harm{\Mean}[\pLevBF_{t}]}_{=1} + \cv_{t+1}(a_{t+1}, \pLevBF_{t+1})                      
% \\   \mathbf{A}_{t+1} & = \Harm{\Mean}_{t+1}[a_{t+1}\pLevBF_{t+1}] = \Harm{\Mean}_{t+1}[a_{t+1}]\Harm{\Mean}_{t+1}[\pLevBF_{t+1}] + \HarmWide{\cv}_{t+1}(a_{t+1}, \pLevBF_{t+1}).
%0 = \mathbf{A}_{t+1}-\PGro \mathbf{A}_{t} & =  \Mean[\aRat_{t+1}]\PGro + \PGro \cov_{t+1} - (\Mean[\aRat_{t}]+\cov_{t}) \\ 
%\left(\Mean[\aRat_{t+1}] \PGro+ \cov_{t+1} \right) & = \left(\Mean[\aRat_{t}]+\cov_{t}\right)  \\ 
%\left(\Mean[\aRat_{t+1}]  - \Mean[\aRat_{t}]\right)\PGro & =\cov_{t+1}  - \PGro \cov_{t} \label{eq:MeanaRatVsCovGro}%
%  \\ \left(\Mean[\aRat_{t+2}]\PGro^{2}  - \Mean[\aRat_{t+1}]\PGro^{1}\PGro^{1}\right) & = \cov_{t+2}  - \PGro \cov_{t+1} \label{eq:MeanaRatVsCovGro2}%
%  \\ \left(\Mean[\aRat_{t+2}]\PGro^{2}  - \Mean[\aRat_{t+1}]\PGro^{2}\right) & = \cov_{t+2}  - \PGro \cov_{t+1} \label{eq:MeanaRatVsCovGro2}%
 \left(\bar{\aRat}_{t+2} - \bar{\aRat}_{t+1}\right)\PGro^{2} & = \cov_{t+2}  - \PGro \cov_{t+1} \label{eq:MeanaRatVsCovGro2}%                                                                                                                                                                       
%  \left(\tilde{\Mean}[\aRat_{t+1}]/\PGro  - \tilde{\Mean}[\aRat_{t}]\right)\PGro & =\cov_{t+1}  - \PGro \cov_{t} \\
%  \bar{\tilde{\aRat}} \left(1  - \PGro\right) & =\cov_{t+1}  - \PGro \cov_{t}
\end{align*}

The less interesting possibility here is when the economy is Szeidl-invariant, in which case $\bar{\aRat}_{t+2} = \bar{\aRat}_{t+1}$, implying that the economy exhibits balanced growth in the covariance:
\begin{align}
 \cov_{t+2} & = \PGro \cov_{t+1}.
\end{align}

The slightly more interesting case is when the economy is Harmenberg- but not Szeidl-invariant.  In that case, if the $\cv$ and the $\Mean[\aRat]$ terms have constant growth factors $\mu_{\cov}$ and $\mu_{\aRat}$, an equation corresponding to \eqref{eq:atp2vsatp1} will hold in $t+n$:
\begin{align}
  (\overbrace{\mu_{\aRat}^{n}\bar{\aRat}_{t}}^{\bar{\aRat}_{t+n}}-\mu^{n-1}_{\aRat}\bar{\aRat}_{t})\PGro^{n} & = (\mu_{\cv}^{n}-\PGro \mu_{\cv}^{n-1}) \cv_{t} \notag
\\ (\mu_{\aRat}\PGro)^{n-1} (\mu_{\aRat}-1)\bar{\aRat}_{t}\PGro & = \mu_{\cv}^{n-1}(\mu_{\cv}-\PGro ) \cv_{t} \label{eq:aRatGrovsCovGronm1}
\end{align}
so for the LHS and RHS to grow at the same rates we need
\begin{align}
 \mu_{\cov}  & = \mu_{\aRat}\PGro \label{eq:aRatGrovscovGro}. 
\end{align}
This is intuitive.  In the Szeidl-invariant economy, this just reproduces our result above that the covariance exhibits balanced growth because $\mu_{\aRat}=1$.  The revised result just says that in the Harmenberg case where the mean value of $\aRat$ can grow, the covariance must rise in proportion to any ongoing expansion of $\bar{\aRat}$ as well.
\begin{comment}
  \begin{align*}
    \overbrace{\mu_{\aRat}^{n} \bar{\aRat}_{t}}^{\bar{\aRat}_{t+n}}(\mu_{\aRat} - 1)\PGro^{n} & =(\mu_{\cv} - \PGro) \overbrace{\cv_{t}\mu_{\cv}^{n}}^{\cv_{t+n}}
\\                                                                                                 
    (\mu_{\aRat}\PGro/\mu_{\cv})^{n} \bar{\aRat}_{t}(\mu_{\aRat} - 1) & =(\mu_{\cv} - \PGro) \cv_{t}
  \end{align*}
    and a corresponding equation will hold in $t+1$:
  \begin{align*}
\\  \overbrace{\mu_{\aRat} \bar{\aRat}_{t}}^{\bar{\aRat}_{t+1}}(\mu_{\aRat} - 1)\PGro & =(\mu_{\cv} - \PGro) \overbrace{\cv_{t}\mu_{\cv}}^{\cv_{t+1}}
  \end{align*}
  \begin{align*}
  (\mu_{\aRat} - 1)\bar{\aRat}_{t}(\mu_{\aRat} - 1)\PGro & =(\mu_{\cv} - \PGro) \cv_{t}(\mu_{\cv} - 1)
\end{align*}
...
\end{align*}
\begin{align}
%  \mu_{\aRat} - 1 & = \mu_{\cv} - \PGro \\
%  \mu_{\aRat} + \pGro & = \mu_{\cv}  \\
  \mu_{\cv} & = \mu_{\aRat} + (\PGro - 1) \label{eq:aRatGroVscov}
\end{align}
\begin{align*}
  \mathbf{A}_{t}  % & l= \bar{\tilde{\aRat}}_{t} = \Harm{\Mean}_{t+1}[a_{t}\pLevBF_{t}] = \Harm{\Mean}_{t+1}[a_{t}]\Harm{\Mean}_{t+1}[\pLevBF_{t}] + \cv_{t}(\aRat_{t}, \pLevBF_{t}) \\
                    & = %\bar{\tilde{\aRat}}_{t} =
                      \Harm{\Mean}_{t\phantom{+1}}[\aRat]\phantom{\PGro} = \Mean[a_{t\phantom{t+1}}]\phantom{\PGro} + \cv_{t}(\aRat_{t}, \pLevBF_{t})  \\
  \mathbf{A}_{t+1} & = \Harm{\Mean}_{t         +1 }[\aRat]         \PGro  = \bar{\aRat}_{t+1}\PGro  + \cv_{t+1}(\aRat_{t+1}, \pLevBF_{t+1})
\end{align*}
...
As Harmenberg points out, a convenient feature of his measure is that $\Harm{\Mean}_{t+n}[\pLevBF_{t+n}]=\PGro^{n}$.  Using this, if by date $t=0$ the economy had achieved a Harmenberg-invariant state then we could define $\bar{\aRat}=\Harm{\Mean}_{t}[a_{t}]$ and use the fact that thereafter assets grow at the constant rate $\PGro$ to obtain
  ...
  \bar{\aRat}\PGro +\HarmWide{\cv}(\aRat_{t+1},\pLevBF_{t+1})   & = \PGro \left( \bar{\aRat}  +  \HarmWide{\cv}(\aRat_{t},\pLevBF_{t})\right) \label{eq:covProblem} 
\\  \HarmWide{\cv}(\aRat_{t+1},\pLevBF_{t+1})   & = \HarmWide{\cv}(\aRat_{t},\pLevBF_{t})  \notag
\end{align}
...
But this discussion also highlights an uncomfortable point: In the model as specified, permanent income does not have a limiting distribution; it becomes ever more dispersed as the economy with infinite-horizon consumers continues to grow indefinitely.
...
\begin{align}
    \cv(\aRat_{t+1},\pLevBF_{t+1})   & = \cv(\aRat_{t},\pLevBF_{t}) \label{eq:covSolnStable} .
\end{align}
\end{comment}

A few microeconomic data sources attempt direct measurement of `permanent income'; \cite{cstwMPC}, for example, show that their assumptions about the magnitude of permanent shocks (and mortality; see below) yield a distribution of permanent income that roughly matches answers in the \emph{Survey of Consumer Finances} to a survey question that aims to elicit a direct measure of permanent income from respondents.  The paper uses those results (and others) to argue that a model that matches empirical facts about the distribution of permanent income and wealth also does a reasonable job of matching empirical facts about the marginal propensity to consume.  The quantitative credibility of the paper's argument depends in good measure on the model's match to the distribution of permanent income inequality, which would not be possible in a model without a nondegenerate steady-state distribution of permanent income.
