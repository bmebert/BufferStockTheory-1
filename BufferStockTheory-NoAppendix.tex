...
% \date{\today}
\renewcommand{\forcedate}{December 17, 2021}\date{\forcedate}
  ...
  \parbox{0.9\textwidth}{
    \begin{center}
      \begin{tabbing}
        ...
        \texttt{~Appendix:~} \> \> \texttt{\href{{https://\owner.github.io/BufferStockTheory/BufferStockTheory3.html}\#Appendices}{{https://\owner.github.io/BufferStockTheory/BufferStockTheory3.html}\#Appendices}}    \\
...
This paper's first contribution is to articulate conditions under which the infinite-horizon Friedman-Muth(-Zeldes) problem (without complications like a consumption floor or liquidity constraints) defines a contraction mapping whose limit is \href{{https://\owner.github.io/BufferStockTheory/BufferStockTheory3.html}#useful}{useful} (neither zero nor $\infty$ everywhere) as the horizon recedes.  A \hyperlink{FVAC}{`Finite Value of Autarky Condition'} is mostly sufficient (the \hyperlink{WRIC}{`Weak Return Impatience Condition'}\footnote{This is a generalization of a condition in~\cite{mstIncFluct}.} is unlikely to bind).  Because the infinite-horizon solution is the limit of finite-horizon recursions, many intermediate results are also applicable to finite-horizon problems.
...
Figures~\ref{fig:cNrmTargetFig}-\ref{fig:cFuncBounds} capture the main properties of the converged consumption rule when the \RIC, \GICNrm, and \FHWC~all hold.\footnote{These figures reflect the converged rule corresponding to the parameter values indicated in Table~\ref{table:Calibration}.}
...
% Given the terminology developed earlier, the natural designation for the stable ratio of aggregate market resources to noncapital income is the `aggregate steady state' ratio, $\BalGroFac{M}$.  This is not numerically the same as the individual \href{{https://\owner.github.io/BufferStockTheory/BufferStockTheory3.html}#pseudo-steady-state}{pseudo-steady-state} ratio $\mBalLvl$ because the nonlinearities involved in simulation and aggregation will have consequences.\footnote{Still, the pseudo-steady-state can be calculated immediately from the policy function without any simulation, and therefore would likely serve as an excellent and low-cost starting point for the numerical simulation process.}
...
Even more interesting is the fact that if \eqref{eq:GICHarm} holds, the economy will exhibit an \textit{aggregate} `steady-state' value of $\MBalLvl$ (different from the individual `pseudo-steady-state' $\mBalLvl$) -- and that so long as \eqref{eq:GICHarm} holds, such an aggregate steady-state $\MBalLvl$ exists even if the {\GICRaw} fails so that the idiosyncratic pseudo-steady-state does not exist.
...
In the special case of a Szeidl-invariant economy, $\cNrmAvg_{t+2} = \cNrmAvg_{t+1}$, implying that the economy exhibits balanced growth in the covariance:
\begin{align}
...
\end{align}
This is intuitive.  In the Szeidl-invariant economy, this just reproduces our result above that the covariance exhibits balanced growth because $\GroFac_{\cNrmAvg}=1$.  The revised result just says that in the Harmenberg case where the mean value $\cNrmAvg$ of the consumption ratio $\cNrm$ can grow, the covariance must rise in proportion to any ongoing expansion of $\cNrmAvg$ (as well as in proportion to the growth in $\PermLvl$).
