...
% \date{\today}
\renewcommand{\forcedate}{December 18, 2021}\date{\forcedate}
...
\begin{align}
  \text{GIC-Szeidl:~~}  \log \GPFacRaw & < \Ex [\log \PermShk] \label{eq:GICSzeidl}
\end{align}
...
Even more interesting is the fact that if \eqref{eq:GICHarm} holds, the economy will exhibit an \textit{aggregate} `steady-state' ratio of market resources to income $\MBalLvl$ (different from the individual `pseudo-steady-state' $\mBalLvl$) -- and that so long as \eqref{eq:GICHarm} holds, such an aggregate steady-state $\MBalLvl$ exists even if the {\GICRaw} fails so that the idiosyncratic pseudo-steady-state does not exist.
...
Harmenberg notes that the covariance between the individual consumption ratio $\cNrm$ and the idiosyncratic component of permanent income $\PermLvl$ does not shrink to zero over time; this implies that covariances constitute another potential measurement that can be used for construction of microfoundations.  % Here we draw out some of those implications.
