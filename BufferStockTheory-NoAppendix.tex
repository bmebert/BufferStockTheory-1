...
% \date{\today}
\renewcommand{\forcedate}{December 6, 2021}\date{\forcedate}
...
\end{align*}
exhibits relative risk aversion $\CRRA > 1$.\footnote{The main results also hold for logarithmic utility which is the limit as $\CRRA \rightarrow 1$ but incorporating the logarithmic special case in the proofs is omitted because it would be cumbersome.}  The consumer's initial condition is defined by market resources $\mLvl_{t}$ and permanent noncapital income $\pLvl_{t}$, which both are positive,
\begin{align}
  \{\pLvl_{t},\mLvl_{t}\} & \in (0,\infty) \label{eq:pLvlandmLvlgt0},
\end{align}
...
We establish a bit more notation by reviewing the familiar result that in such problems (CRRA utility, permanent shocks) the number of states can be reduced from two ($\mLvl$ and $\pLvl$) to one $(\mNrm = \mLvl/\pLvl)$.  Value in the last period is $\uFunc(\mLvl_{T})$; using (in the last line in~\eqref{eq:vBold}) the fact that for our CRRA utility function, $\uFunc(xy) = x^{1-\CRRA}\uFunc(y)$, and generically defining nonbold variables as the boldface counterpart normalized by $\pLvl_{t}$ (as with $\mNrm=\mLvl/\pLvl$), consider the problem in the second-to-last period,
\begin{equation}\label{eq:vBold}
  \begin{split}
    \vLvl_{T-1}(\mLvl_{T-1},\pLvl_{T-1})
    & =  \max_{\cNrm_{T-1}}~
    \uFunc(\pLvl_{T-1}\cNrm_{T-1}) + \DiscFac  \Ex_{T-1} [\uFunc(\pLvl_{T}{\mNrm}
    _{T})]  \\
    & = \pLvl_{T-1}^{1-\CRRA}
    \left\{\max_{\cNrm_{T-1}}~ \uFunc(c_{T-1}) + \DiscFac \Ex_{T-1} [ \uFunc( {\PGro}_{T}
      {\mNrm}_{T}) ] \right\}.
  \end{split}
\end{equation}
...
\begin{align*}
  \vLvl_{T-1}(\mLvl_{T-1},\pLvl_{T-1})  & = \pLvl_{T-1}^{1-\CRRA} \vFunc_{T-1}(\underbrace{\mLvl_{T-1}/\pLvl_{T-1}}_{=\mNrm_{T-1}}).
\end{align*}
...
\begin{align*}
  \vLvl_{t}(\mLvl_{t},\pLvl_{t})  & = \pLvl_{t}^{1-\CRRA}\vFunc_{t}(\mNrm_{t}),
  \\ \cLvl_{t}(\mLvl_{t},\pLvl_{t})  & = \pLvl_{t}\cFunc_{t}(\mNrm_{t}).
\end{align*}
...
\begin{align}
  \mathrm{PDV}_{t}(\cLvl)  & = \overbrace{\mLvl_{t}-\pLvl_{t}}^{\bLvl_{t}}+\overbrace{\mathrm{PDV}_{t}(\pLvl)}^{\hLvl_{t}}, \label{eq:IBCFinite}
\end{align} \hypertarget{FHWF}{}
where $\bLvl$ is beginning-of-period `market' balances, with$\Rnorm \equiv \Rfree/\PGro$ `human wealth' is
\begin{align}
  \hLvl_{t}  & = \pLvl_{t}+\Rnorm^{-1} \pLvl_{t} + \Rnorm^{-2} \pLvl_{t} + \cdots + \Rnorm^{t-T} \pLvl_{t} \notag
  \\  & = \underbrace{\left(\frac{1-\Rnorm^{-(T-t+1)}}{1-\Rnorm^{-1}}\right)}_{\equiv \hNrm_{t}}\pLvl_{t} \label{eq:HDef}.
...
A `Return Patience Factor' ({\RPFDfn}) relates absolute patience to the return factor:
\begin{align}
\text{\RPFDfn:~~}  \PatR  & \equiv  {\Pat}/\Rfree \phantom{\text{\RPF:~~}}\label{eq:PatR}
\end{align}
and since consumption is growing by $\Pat$ but discounted by $\Rfree$:
\begin{align}
  \mathrm{PDV}_{t}(\cLvl)  & = \left(\frac{1-\PatR^{T-t+1}}{1-\PatR}\right)\cLvl_{t}
\end{align}
from which the IBC~\eqref{eq:IBCFinite} implies
\begin{align}
  \cLvl_{t}  & = \overbrace{\left(\frac{1-\PatR}{1-\PatR^{T-t+1}}\right)}^{\equiv \MinMPC_{t}}
...
\end{align}
where $\MinMPC_{t}$ is the marginal propensity to consume (MPC) --- it answers the question `if the consumer had an extra unit of resources, how much more spending would occur?'  ($\bar{\cFunc}$'s overbar signifies that $\bar{\cFunc}$ will be an upper bound as we modify the problem to incorporate constraints and uncertainty; analogously, $\MinMPC$ is a lower bound for the MPC).

The denominator of~\eqref{eq:WDef} is why, for $\underbar{\MPC}$ to be strictly positive as $n=T-t$ goes to infinity, we must impose the Return Impatience Condition:
\begin{align}
...
\hypertarget{GPFRawDfn}{}\hypertarget{GIC}{}
\subsubsection{The Growth Impatience Condition ({\GICRaw})}\label{subsubsec:GIC}
By analogy to the \RPFDfn, we define a `growth patience factor' as
\begin{equation}\begin{gathered}\begin{aligned}
 \text{{\GPFRawDfn}:~~} && \PatPGro= {\Pat}/\PGro,  &   \label{eq:GPFRaw}
...
Under `autarky,' capital markets do not exist; the consumer has no choice but to spend permanent noncapital income $\pLvl$ in every period.  Because $\uFunc(xy) = x^{1-\CRRA}\uFunc(y)$, the value the consumer would achieve is
%\begin{equation}
%  \begin{split}
\begin{equation}\begin{gathered}\begin{aligned}  \label{eq:ValuePFAnalyticalAutarky}
    \vLvl_{t}^{\text{autarky}}  & = \uFunc(\pLvl_{t})+\DiscFac \uFunc(\pLvl_{t}\PGro)+\DiscFac^{2} \uFunc(\pLvl_{t} \PGro^{2})+\ldots
      \\  & = \uFunc(\pLvl_{t})\left(\frac{1-{(\DiscFac \PGro^{1-\CRRA})}^{T-t+1}}{1-\DiscFac \PGro^{1-\CRRA}}\right) \notag
\end{aligned}\end{gathered}\end{equation}
...
If the {\FHWC} is satisfied, the {\PFFVAC} implies that the {\RIC} is satisfied.\footnote{Divide both sides of the second inequality in~\eqref{eq:PFFVAC} by $\Rfree$:
  ...
  \end{align}
  and {\FHWC} $\Rightarrow$ the RHS is $< 1$ because $(\PGro/\Rfree) < 1$ (and the RHS is raised to a positive power (because $\CRRA>1$)).}  Likewise, if the {\FHWC} and the {\GICRaw} are both satisfied, {\PFFVAC} follows:
...
This logic holds even when the finite human wealth condition fails (\cncl{\FHWC}), because the constraint prevents the (limiting) consumer\footnote{That is, one obeying $\cFunc(\mNrm) = \lim_{n \uparrow \infty} \cFunc_{T-n}(\mNrm)$.} from borrowing against unbounded human wealth to finance unbounded current consumption.  Under these circumstances, the consumer who starts with any $\bNrm_{t} > 0$ will, over time, run those resources down so that by some finite number of periods $\tau$ in the future the consumer will reach $\bNrm_{t+\tau} = 0$, and thereafter will set $\cLvl = \pLvl$ for eternity (which the {\PFFVAC} says yields finite value).  Using the same steps as for equation~\eqref{eq:ValuePFAnalyticalAutarky}, value of the interim program is also finite: \hypertarget{PFFVAC}{} \hypertarget{PFVAFDfn}{}
\begin{align*}
  \vLvl_{t+\tau} 
  & = \PGro^{\tau(1-\CRRA)} \uFunc(\pLvl_{t})\left(\frac{1-{(\DiscFac \PGro^{1-\CRRA})}^{T-(t+\tau)+1}}{1-\DiscFac \PGro^{1-\CRRA}}\right).
\end{align*}
...
\begin{align*}
  \Ex_{t}[\bNrm_{t+1}]  & =  \aNrm_{t}\Ex_{t}[(\Rfree/\PGro_{t+1})] = \aNrm_{t}(\Rfree/\PGro)\Ex_{t}[\pShk_{t+1}^{-1}] %\label{eq:EbNrm}
\end{align*}
...
\hypertarget{GICNrmI}{}\hypertarget{PermGroAdj}{}\hypertarget{GPFNrmDfn}{}
\begin{verbatimwrite}{\EqDir/PatPermGroFacAdj}
  \begin{align}
\text{{\GPFNrmDfn}:~~}    \PatPermGroFacAdj  & = \Pat/\PermGroFacAdj = \Ex[\Pat/(\PGro\pShk)]  \phantom{\text{{\GPFNrm}:~~}} \label{eq:GPFNrm}
  \end{align}
\end{verbatimwrite}
\input{\LtxDir\EqDir/PatPermGroFacAdj}
\begin{comment}
  and the proof of Theorem~\ref{thm:MSSBalExists} below yields the conclusion that
  \begin{align*}
    \lim_{\mNrm_{t} \rightarrow \infty} \Ex_{t}[\mNrm_{t+1}/\mNrm_{t}]  & = \PatPermGroFacAdj,
  \end{align*}
...
that is stronger than the perfect foresight version~\eqref{eq:GICRaw} because $\PermGroFacAdj < \PGro$.
\begin{comment}
  \begin{align}
    \PermGroFacAdj & < \PGro \label{eq:PermGroFacAdjLTPGro}.
  \end{align}
\end{comment}
...
\begin{align*}
  \vLvl_{t}  & = \Ex_{t}\left[\uFunc(\pLvl_{t}) + \DiscFac \uFunc(\pLvl_{t}\PGro_{t+1}) + \cdots + \DiscFac^{T-t}\uFunc(\pLvl_{t}\PGro_{t+1}\ldots\PGro_{T})\right] \\
  % & = \uFunc(\pLvl_{t})\left(1+ \DiscFac \Ex_{t}[\PGro_{t+1}^{1-\CRRA}] +  \ldots + \DiscFac^{T-t}\Ex_{t}[\PGro_{t+1}^{1-\CRRA}]\ldots\Ex_{t}[\PGro_{T}^{1-\CRRA}]\right) \\
               & = \uFunc(\pLvl_{t})\left(\frac{1-{(\DiscFac \PGro^{1-\CRRA}\Ex[\pShk^{1-\CRRA}])}^{T-t+1}}{1-\DiscFac \PGro^{1-\CRRA} \Ex[\pShk^{1-\CRRA}]}\right),
\end{align*}
...
\begin{equation}
  ...
  \PermGroFacAdj = \PGro \uInvEpShkuInv,
\end{equation}
...
\hypertarget{WRIC}{}\hypertarget{WRPFDfn}{}
...
First, as $\mNrm_{t} \uparrow \infty$ the expected consumption growth factor goes to ${\Pat}$, indicated by the lower bound in Figure~\ref{fig:cGroTargetFig}, and the marginal propensity to consume approaches $\MinMPC=(1-\PatR)$ (see Figure~\ref{fig:mpclimits}) --- the same as the perfect foresight MPC.\@  Second, as $\mNrm_{t}$ approaches zero the consumption growth factor approaches $\infty$ (Figure~\ref{fig:cGroTargetFig}) and the MPC approaches $\MaxMPC=(1-\pZero^{1/\CRRA}\PatR)$ (Figure~\ref{fig:mpclimits}).  Third, there is a value of the market resources ratio $\mNrm_{t} = \mBalLvl$ at which the expected growth rate of the level of market resources $\mLvl$ matches the expected growth rate of permanent income $\PGro$, and a different (larger) target ratio $\mTrgNrm$ where $\Ex[\mNrm_{t+1}/\mNrm_{t}]=1$ (and the expected growth rate of consumption is lower than $\PGro$).  Thus, at the individual level, this model does not have a single $\mNrm$ at which $\pLvl,~\mLvl,~$ and $\cLvl$ all are expected to grow at the same rate.
...
\end{equation*}
because any other fixed limit would eventually lead to a level of consumption either exceeding $\bar{\cFunc}(\mNrm)$ or lower than $\ushort{\cFunc}(\mNrm)$.
...
\begin{align*}
  ...
  \cLvl_{t+1}/\cLvl_{t}]  & = \lim_{\mNrm_{t} \uparrow \infty} \Ex_{t}[
                                {\PGro}_{t+1} {\cNrm}_{t+1}/c_{t}].
\end{align*}

But
\begin{align*}
  \Ex_{t}[{\PGro}_{t+1} {\ushort{\cNrm}}_{t+1}/\bar{\cNrm_{t}}] \leq \Ex_{t}[{\PGro}_{t+1} {\cNrm}_{t+1}/\cNrm_{t}] \leq \Ex_{t}[{\PGro}_{t+1} {\bar{\cNrm}}_{t+1}/\ushort{\cNrm}_{t}]
\end{align*}
and
\begin{equation*}  \label{eq:xttoinfty}
  \lim_{\mNrm_t \uparrow \infty} \PGro_{t+1}\ushort{\cFunc}(\mNrm_{t+1})/\bar{\cFunc}(\mNrm_t) =
  \lim_{\mNrm_{t} \uparrow \infty} \PGro_{t+1}\bar{\cFunc}(\mNrm_{t+1})/\ushort{\cFunc}(\mNrm_t) =
  \lim_{\mNrm_{t} \uparrow \infty}\PGro_{t+1} \mNrm_{t+1}/\mNrm_t,
\end{equation*}
...
\begin{align}  \label{eq:xtp1toinfty}
  \lim_{\mNrm_{t} \uparrow \infty} \PGro_{t+1} \mNrm_{t+1}/\mNrm_t  & = \lim_{\mNrm_{t} \uparrow \infty}
                                                                      \left(\frac{\Rfree \aFunc(\mNrm_t)+{\PGro}_{t+1}\tShkAll_{t+1}}{\mNrm_t}\right)
  \\  & = {(\Rfree \DiscFac)}^{1/\CRRA} = \Pat \notag
\end{align}
because $\lim_{\mNrm_{t}\uparrow \infty} \aFunc^{\prime}(\mNrm)=\PatR$\footnote{This is because $\lim_{\mNrm_{t}\uparrow \infty} \aFunc(\mNrm_{t})/\mNrm_{t}=1-\lim_{\mNrm_{t}\uparrow \infty} \usual{\cFunc}(\mNrm_{t})/\mNrm_{t}=1-\lim_{\mNrm_{t}\uparrow \infty}\usual{\cFunc}^{\prime}(\mNrm_{t})=\PatR$.} and $\PGro_{t+1}\tShkAll_{t+1}/\mNrm_{t} \leq (\PGro \bar{\pShk} \bar{\tShkEmp}/\pNotZero )/\mNrm_{t}$ which goes to zero as $\mNrm_{t}$ goes to infinity.

Hence we have
\begin{equation*}
  {\Pat}  \leq \lim_{\mNrm_{t} \uparrow \infty} \Ex_{t}[\cLvl_{t+1}/\cLvl_{t}] \leq {\Pat}
\end{equation*}
...
\begin{align*}
  \lim_{m \downarrow 0} \eFunc(\mNrm)  & = (1-\pZero^{1/\CRRA}\PatR) = \MaxMPC .
\end{align*}
...
Since $\mNrm_{t+1}=(\mNrm_{t}-\cFunc(\mNrm_{t}))\Rnorm_{t+1}+\tShk_{t+1}$, the implicit equation for $\mTrgNrm$ is
...
\begin{verbatimwrite}{\EqDir/TheoremMSSBalExists}
  \begin{theorem}\label{thm:MSSBalExists}
    ...
    \begin{equation}
      \Ex_t [\pShk_{t+1}{\mNrm}_{t+1}/\mNrm_t] = 1 \mbox{~if~} \mNrm_t = \mBalLvl.
      \label{eq:mBalLvl}
    \end{equation}
    Moreover, $\mBalLvl$ is a point of stability in the sense that
    \begin{equation}\label{eq:stabilityStE}
      \begin{split}
        \forall {\mNrm}_t\in(0,\mBalLvl),      \,\,& \Ex_{t}[\mLvl_{t+1}]/\mLvl_{t} > \PGro \\
        \forall {\mNrm}_t\in(\mBalLvl,\infty), \,\,& \Ex_{t}[\mLvl_{t+1}]/\mLvl_{t} < \PGro.
      \end{split}
    \end{equation}
  \end{theorem}
\end{verbatimwrite}
...
\begin{enumerate}
\item Existence and continuity of $\Ex_{t}[\mNrm_{t+1}/\mNrm_{t}]$ or $\Ex_{t}[\mNrm_{t+1}\pShk_{t+1}/\mNrm_{t}]$
  \begin{itemize}
...
\item Existence of the equilibrium point
  ...
  \end{itemize}
\item Monotonicity of $\Ex_{t}[{\mNrm}_{t+1}-\mNrm_{t}]$ or $\Ex_{t}[{\mNrm}_{t+1}\pShk_{t+1}-\mNrm_{t}]$
  \begin{itemize}
...
\end{align}
which is stronger than our {\GICRaw} condition $\PatPGro < 1$ (under the imposed assumption $\Ex[\log \pShk]<0$).\footnote{Under our default (though not required) assumption that $\log \pShk \sim \mathcal{N}(-\sigma^{2}_{\pShk}/2,\sigma^{2}_{\pShk})$; the {\GICNrm} in this case, is $\PatPGro < \exp(-\sigma^{2})$, so if the {\GICNrm} holds then Szeidl's condition will hold.}  %Below we designate these the `Szeidl-invariant' distributions $\Nvrnt({\mNrm})$, $\Nvrnt({\aNrm})$, and $\Nvrnt({\cNrm})$.%\footnote{Szeidl's proof supplants simulation evidence of ergodicity that appeared in an earlier version of this paper.}
...
\cite{harmenbergInvariant} substitutes a clever change of probability-measure into Szeidl's proof to show that under the {\GICRaw}, invariant \emph{permanent-income-weighted} distributions exist.\footnote{Harmenberg in a nutshell:  If $\pShk$ is described by density function $f_\pShk(\pShk)$, define the \emph{permanent-income-neutral measure} by $\Harm{f}_\pShk(\pShk) \equiv \pShk f_\pShk(\pShk)$.  Following derivations parallel to those in the previous foonote, the condition for the existence of invariant permanent-income-weighted distributions is
  ...
  \end{align*}
  where the $\sim$ over the expectations operator indicates that the expectation is being taken with respect to $f_\pShk(\pShk)$.  But by assumption $\Harm{\Ex} \log \pShk = 0$ so exponentiating both sides of this equation turns it into $\PatPGro < 1$ which is the {\GICRaw}.}  This allows him to prove a conjecture from an earlier draft of this paper (\cite{BufferStockTheoryQESubmit}) that under the {\GICRaw}, aggregate consumption grows at the same rate $\PGro$ as aggregate noncapital income in the long run (with the corollary that aggregate assets and market resources grow at that same rate). \hypertarget{test-Harmenbergs-method}{}
...
\begin{align*}
  \YLvl_{t+1}/\YLvl_{t}  & = \Mean\left[\tShkAll_{t+1}\PGro \pShk_{t+1}\pLvl_{t}\right]/\Mean\left[\pLvl_{t}\tShkAll_{t}\right]  \\
                             & = \PGro
\end{align*}
...
\begin{equation}\label{eq:MeanDeltaLogC}
  \begin{split}
    \Mean\left[\Delta \log \cLvl_{t+1}\right]  & = \Mean\left[ \log {\cNrm}_{t+1}\pLvl_{t+1} - \log c_{t}\pLvl_{t}\right]  \\
    % & = \Mean\left[ \log \pLvl_{t+1}- \log \pLvl_{t} + \log {\cNrm}_{t+1} - \log c_{t}\right]  \notag \\
    & = \Mean\left[ \log \pLvl_{t+1}- \log \pLvl_{t}\right] + \Mean\left[ \log {\cNrm}_{t+1} - \log c_{t}\right].
  \end{split}
\end{equation}
...
\begin{align}
  \Mean\left[\Delta \log \cLvl_{t+1}\right]  & = \Mean\left[ \Delta \log \pLvl_{t+1}\right] = \log \PGro \label{eq:MeanDeltaLogCeqMeanDeltaLogP}.
\end{align}

In a Harmenberg-invariant economy (and therefore also any Szeidl-invariant economy), a similar proposition holds in the cross-section as a direct implication of the fact that a constant proportion of total permanent income is accounted for by the successive sets of consumers with any particular $\mNrm$.  This fact is one way of interpreting Harmenberg's definition of the density of the permanent-income-invariant distribution of $\mNrm$; call this density~$\pNrm(\mNrm)$.\footnote{In his notation, this is $\tilde{\psi}^{\mNrm}$.}  Call $\cLvl_{t}(\mNrm)$ the total amount of consumption at date $t$ by persons with market resources $\mNrm$, and note that in the invariant economy this is given by the converged consumption function $\cFunc(\mNrm)$ multiplied by the amount of permanent income accruing to such people $\pNrm(\mNrm)\PLvl_{t}$.  Since $\pNrm(\mNrm)$ is invariant and aggregate permanent income grows according to $\PLvl_{t+1} = \PGro \PLvl_{t}$,
\begin{equation*}
  \begin{split}
    \log \cLvl_{t+1}(\mNrm) - \log \cLvl_{t}(\mNrm) &  \notag
    = \log \cFunc(\mNrm) \pNrm(\mNrm)\PLvl_{t+1} - \log \cFunc(\mNrm)\pNrm(\mNrm)\PLvl_{t} \\
    & = \log \PGro
  \end{split}
\end{equation*}
...
Harmenberg has a brief discussion demonstrating that the covariance between the individual consumption ratio $\cNrm$ and the idiosyncratic component of permanent income $\pLvl$ does not shrink to zero over time; so covariances constitute another potential measurement that can be used for construction of microfoundations.  % Here we draw out some of those implications.

Consider a date-$t$ Harmenberg-converged economy, and define the mean value of the consumption ratio as $\cRatAvg_{t+n} \equiv \Mean[\cNrm_{t+n}]$.  Normalizing period-$t$ aggregate permanent income to $\PLvl_{t}=1$, total consumption at $t+1$ and $t+2$ are
\begin{equation}\label{eq:atp2vsatp1}
  \begin{split}
    % \CLvl_{t}  % & l= \bar{\tilde{\cNrm}}_{t} = \Mean[\cNrm_{t}\pLvl_{t}] = \Mean[\cNrm_{t}]\Mean[\pLvl_{t}] + \cv_{t}(\cNrm_{t}, \pLvl_{t}) \\
    % & = %\bar{\tilde{\cNrm}}_{t} =
    % \Mean[\cNrm_{t}\pLvl_{t}] = \Mean[\cNrm_{t}]\overbrace{\Mean[\pLvl_{t}]}^{=1} + \cv_{t}(\cNrm_{t}, \pLvl_{t})  \\
    \CLvl_{t+1} & = \Mean[\cNrm_{t+1}\pLvl_{t+1}] = \cRatAvg_{t+1}\PGro^{1} + \cv_{t+1}(\cNrm_{t+1}, \pLvl_{t+1})
    \\  \CLvl_{t+2} & = \Mean[\cNrm_{t+2}\pLvl_{t+2}] = \cRatAvg_{t+2}\PGro^{2} + \cv_{t+2}(\cNrm_{t+2}, \pLvl_{t+2})
  \end{split}
\end{equation}
and Harmenberg's proof that $\CLvl_{t+2}-\PGro \CLvl_{t+1}=0$ allows us to obtain:
...
\begin{equation} \label{eq:aNrmGrovsCovGronm1}
  \begin{split}
    (\overbrace{\GroFac_{\cRatAvg}^{n}\cRatAvg_{t}}^{\cRatAvg_{t+n}}-\GroFac^{n-1}_{\cNrm}\cRatAvg_{t})\PGro^{n} & = \left(\PGro\GroFac_{\cv}^{n-1} -\GroFac_{\cv}^{n}\right)\cv_{t}
...
\begin{align}
  \GroFac_{\cov}  & = \GroFac_{\cRatAvg}\PGro \label{eq:aNrmGrovscovGro}.
\end{align}
...
\begin{comment}
  ...
  \begin{align}
    ...
    \GroFac_{\cv} & = \GroFac_{\cRatAvg} + (\PGro - 1) \label{eq:aNrmGroVscov}
  \end{align}
  \begin{align*}
    \CLvl_{t}  % & l= \bar{\tilde{\cNrm}}_{t} = \Harm{\Mean}_{t+1}[\cNrm_{t}\pLvl_{t}] = \Harm{\Mean}_{t+1}[\cNrm_{t}]\Harm{\Mean}_{t+1}[\pLvl_{t}] + \cv_{t}(\cNrm_{t}, \pLvl_{t}) \\
                   & = %\bar{\tilde{\cNrm}}_{t} =
                     \Harm{\Mean}_{t\phantom{+1}}[\cNrm]\phantom{\PGro} = \Mean[\cNrm_{t\phantom{t+1}}]\phantom{\PGro} + \cv_{t}(\cNrm_{t}, \pLvl_{t})  \\
    \CLvl_{t+1} & = \Harm{\Mean}_{t         +1 }[\cNrm]         \PGro  = \cRatAvg_{t+1}\PGro  + \cv_{t+1}(\cNrm_{t+1}, \pLvl_{t+1})
  \end{align*}
...
\begin{comment}
  ...
  As Harmenberg points out, a convenient feature of his measure is that $\Harm{\Mean}_{t+n}[\pLvl_{t+n}]=\PGro^{n}$.  Using this, if by date $t=0$ the economy had achieved a Harmenberg-invariant state then we could define $\cRatAvg=\Harm{\Mean}_{t}[\cNrm_ {t}]$ and use the fact that thereafter assets grow at the constant rate $\PGro$ to obtain
  \begin{equation}
    \begin{split}\label{eq:covProblem}
      \CLvl_{t+1} & = \PGro \CLvl_{t}\notag \\
      \cRatAvg\PGro +\HarmWide{\cv}(\cNrm_{t+1},\pLvl_{t+1})   & = \PGro \left( \cRatAvg    +  \HarmWide{\cv}(\cNrm_{t},\pLvl_{t})\right)
      \\  \HarmWide{\cv}(\cNrm_{t+1},\pLvl_{t+1})   & = \HarmWide{\cv}(\cNrm_{t},\pLvl_{t})  \notag
    \end{split}
  \end{equation}

  A corresponding argument shows that $\cov(\mNrm,\pLvl)$ also grows by $\PGro$.
\end{comment}
