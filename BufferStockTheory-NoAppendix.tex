...
\begin{equation}\label{eq:atp2vsatp1}
  \begin{split}
    ...
    \CLvl_{t+1} & = \Mean[\cNrm_{t+1}\PermLvl_{t+1}] = \cRatAvg_{t+1}\PermGroFac^{1} + \cov_{t+1}(\cNrm_{t+1}, \PermLvl_{t+1})
    \\  \CLvl_{t+2} & = \Mean[\cNrm_{t+2}\PermLvl_{t+2}] = \cRatAvg_{t+2}\PermGroFac^{2} + \cov_{t+2}(\cNrm_{t+2}, \PermLvl_{t+2})
  \end{split}
\end{equation}
...
\begin{equation} \label{eq:cNrmvsCov}\begin{split}
    % 0 & = \cRatAvg_{t+2}\PermGroFac^{2} + \cov_{t+2}-\PermGroFac\left(\PermGroFac\cRatAvg_{t+1} + \cov_{t+1}\right)\\ %
    % \cRatAvg_{t+2}\PermGroFac^{2} + \cov_{t+2}   & =\cRatAvg_{t+1}\PermGroFac^{2} + \PermGroFac \cov_{t+1}\\
...
The more interesting case is when the economy is Harmenberg- but not Szeidl-invariant.  In that case, if the $\cov$ and the $\cRatAvg$ terms have constant growth factors $\GroFac_{\cov}$ and $\GroFac_{\cRatAvg}$,\footnote{This `if' is a conjecture, not something proven by Harmenberg (or anyone else).  But see appendix~\ref{sec:ApndxBalancedGrowthcNrmAndCov} for an example of a Harmenberg-invariant economy in which simulations suggest this proposition holds.} an equation corresponding to \eqref{eq:cNrmvsCov} will hold in $t+n$:
\begin{equation} \label{eq:aNrmGrovsCovGronm1}
  \begin{split}
    (\overbrace{\GroFac_{\cRatAvg}^{n}\cRatAvg_{t}}^{\cRatAvg_{t+n}}-\GroFac^{n-1}_{\cNrm}\cRatAvg_{t})\PermGroFac^{n} & = \left(\PermGroFac\GroFac_{\cov}^{n-1} -\GroFac_{\cov}^{n}\right)\cov_{t}
    \\ (\GroFac_{\cRatAvg}\PermGroFac)^{n-1} (\GroFac_{\cRatAvg}-1)\cRatAvg_{t}\PermGroFac & = \GroFac_{\cov}^{n-1}(\PermGroFac - \GroFac_{\cov}) \cov_{t}
  \end{split}
\end{equation}
...
\begin{comment}
  \begin{align*}
    \overbrace{\GroFac_{\cRatAvg}^{n} \cRatAvg_{t}}^{\cRatAvg_{t+n}}(\GroFac_{\cRatAvg} - 1)\PermGroFac^{n} & =(\GroFac_{\cov} - \PermGroFac) \overbrace{\cov_{t}\GroFac_{\cov}^{n}}^{\cov_{t+n}}
    \\
    (\GroFac_{\cRatAvg}\PermGroFac/\GroFac_{\cov})^{n} \cRatAvg_{t}(\GroFac_{\cRatAvg} - 1) & =(\GroFac_{\cov} - \PermGroFac) \cov_{t}
  \end{align*}
  and a corresponding equation will hold in $t+1$:
  \begin{align*}
    \\  \overbrace{\GroFac_{\cRatAvg} \cRatAvg_{t}}^{\cRatAvg_{t+1}}(\GroFac_{\cRatAvg} - 1)\PermGroFac & =(\GroFac_{\cov} - \PermGroFac) \overbrace{\cov_{t}\GroFac_{\cov}}^{\cov_{t+1}}
  \end{align*}
  \begin{align*}
    (\GroFac_{\cRatAvg} - 1)\cRatAvg_{t}(\GroFac_{\cRatAvg} - 1)\PermGroFac & =(\GroFac_{\cov} - \PermGroFac) \cov_{t}(\GroFac_{\cov} - 1)
  \end{align*}
  ...
  \end{align*}
  \begin{align}
    % \GroFac_{\cRatAvg} - 1 & = \GroFac_{\cov} - \PermGroFac \\
    % \GroFac_{\cRatAvg} + \PermGroRte & = \GroFac_{\cov}  \\
    \GroFac_{\cov} & = \GroFac_{\cRatAvg} + (\PermGroFac - 1) \label{eq:aNrmGroVscov}
  \end{align}
  \begin{align*}
    ...
    \CLvl_{t+1} & = \Harm{\Mean}_{t         +1 }[\cNrm]         \PermGroFac  = \cRatAvg_{t+1}\PermGroFac  + \cov_{t+1}(\cNrm_{t+1}, \PermLvl_{t+1})
  \end{align*}
...
\begin{comment}
  ...
  \begin{equation}
    \begin{split}\label{eq:covProblem}
      ...
      \cRatAvg\PermGroFac +\HarmWide{\cov}(\cNrm_{t+1},\PermLvl_{t+1})   & = \PermGroFac \left( \cRatAvg    +  \HarmWide{\cov}(\cNrm_{t},\PermLvl_{t})\right)
