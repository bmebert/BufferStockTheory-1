...
\end{align*}
\end{comment}
Consider again a Harmenberg-converged economy at date $t=0$, and suppose for convenience that mean permanent income for all consumers at that date happens to be $\Mean[\pLevBF_{t}]=1$.  Total assets at $t$ and $t+1$ are
\begin{align*}
  \mathbf{A}_{t}  % & l= \bar{\tilde{\aRat}}_{t} = \Mean[a_{t}\pLevBF_{t}] = \Mean[a_{t}]\Mean[\pLevBF_{t}] + \textup{cov}_{t}(\aRat_{t}, \pLevBF_{t}) \\
                    & = %\bar{\tilde{\aRat}}_{t} =
                      \Mean[a_{t}\pLevBF_{t}] = \Mean[a_{t}] + \textup{cov}_{t}(\aRat_{t}, \pLevBF_{t})  \\
  \mathbf{A}_{t+1} & = \Mean[a_{t+1}\pLevBF_{t+1}] = \Mean[a_{t+1}]\PGro + \textup{cov}_{t+1}(\aRat_{t+1}, \pLevBF_{t+1})
\end{align*}
but Harmenberg's proof that $\mathbf{A}_{t+1}-\PGro \mathbf{A}_{t}=0$ allows us to use these to conclude that
\begin{align*}
%  \mathbf{A}_{t+1} & = \Mean_{t+1}[a_{t+1}\pLevBF_{t+1}] = \Mean_{t+1}[a_{t+1}\PGro\pShk_{t+1}\pLevBF_{t}] + \textup{cov}_{t+1}(\aRat_{t+1}, \pLevBF_{t}\PGro \pShk_{t+1})
%\\  & = \PGro \Harm{\Mean}_{t+1}[a_{t+1}]\underbrace{\Harm{\Mean}[\pLevBF_{t}]}_{=1} + \textup{cov}_{t+1}(a_{t+1}, \pLevBF_{t+1})                      
...
%\left(\Mean[\aRat_{t+1}] \PGro+ \cov_{t+1}(\aRat_{t+1},\pLevBF_{t+1}) \right) & = \left(\Mean[\aRat_{t}]+\cov_{t}(\aRat_{t},\pLevBF_{t})\right)  \\ 
\left(\Mean[\aRat_{t+1}]  - \Mean[\aRat_{t}]\right)\PGro & =\cov_{t+1}(\aRat_{t+1},\pLevBF_{t+1})  - \PGro \cov_{t}(\aRat_{t},\pLevBF_{t})
\end{align*}

The less interesting possibility here is when the economy is Szeidl-invariant, in which case $\Mean[\aRat_{t+1}] = \Mean[\aRat_{t}] = \bar{\aRat}$, implying that the economy exhibits balanced growth in the covariance:
\begin{align}
 \cov_{t+1}(\aRat_{t+1},\pLevBF_{t+1}) & = \PGro \cov_{t}(\aRat_{t},\pLevBF_{t}).
\end{align}

The more interesting case is when the economy is Harmenberg- but not Szeidl-invariant, in which case all we are entitled to conclude is that any growth in the mean ratio is matched by corresponding growth in the covariance.  Still, both of these quantitites are in priniciple measurable in micro data.  %Indeed, in principle, the Harmenberg-invariant distribution itself could be measured in such data.

\begin{comment}
which is the point where $\HarmWide{\textup{cov}}_{t}$ is the covariance calculated according to the Harmenberg measure.
