...
\newcommand{\inframe}{https://\owner.github.io/BufferStockTheory3.html/}
...
This paper's first contribution is to articulate conditions under which the infinite-horizon Friedman-Muth(-Zeldes) problem (without complications like a consumption floor or liquidity constraints) defines a contraction mapping whose limit is \href{\inframe\#useful}{useful} (neither zero nor $\infty$ everywhere) as the horizon recedes.  A \hyperlink{FVAC}{`Finite Value of Autarky Condition'} is mostly sufficient (the \hyperlink{WRIC}{`Weak Return Impatience Condition'}\footnote{This is a generalization of a condition in~\cite{mstIncFluct}.} is unlikely to bind).  Because the infinite-horizon solution is the limit of finite-horizon recursions, many intermediate results are also useful for solving finite-horizon problems.
...
\end{align}\hypertarget{FHWCDefn}{}
...
\hypertarget{GPFacRawDefn}{}\hypertarget{GIC}{}\hypertarget{GICRawDefn}{}
...
\end{align*}
where Jensen's inequality guarantees that the expectation of the inverse of the permanent shock is greater than one.  Now define\hypertarget{InvEPermShkInvDefn}{}
\begin{align}
...
\hypertarget{PermGroAdjDefn}{}\hypertarget{GPFacNrmDefn}{}
...
\end{align*}
suggesting the definition of a utility-compensated equivalent of the permanent shock,\hypertarget{EuPermShk}{}
\begin{align}
...
\begin{equation}
  ...
  \PermGroFacAdj = \PermGroFac \uInvEuPermShk,
\end{equation}
$\vLvl_{t}$ will be positive and finite as $T$ approaches $\infty$ if\hypertarget{FVACDefn}{}
...
\hypertarget{MPCnvrsUpperDefn}{}
\hypertarget{WRICDefn}{}\hypertarget{WRPFacDefn}{}
Appendix equation~\eqref{eq:MPCmaxInvApndx} presents a parallel expression for the limiting maximal MPC as $\mNrm_{t} \downarrow 0$:
\begin{align}
...
% Given the terminology developed earlier, the natural designation for the stable ratio of aggregate market resources to noncapital income is the `aggregate steady state' ratio, $\BalGroFac{M}$.  This is not numerically the same as the individual \href{\inframe\#pseudo-steady-state}{pseudo-steady-state} ratio $\mBalLvl$ because the nonlinearities involved in simulation and aggregation will have consequences.\footnote{Still, the pseudo-steady-state can be calculated immediately from the policy function without any simulation, and therefore would likely serve as an excellent and low-cost starting point for the numerical simulation process.}
