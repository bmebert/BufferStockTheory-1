...
A somewhat looser proposition is easy to derive in the Harmenberg-invariant economy from the fact that in such an economy a constant proportion of aggregate permanent income is accounted for by the successive sets of consumers with any particular $\mRat$.  This follows because Harmenberg's definition of the permanent-income-invariant distribution of a variable (say, $\mRat$) is that the proportion of permanent income accruing to persons at a specific value of that variable is constant, and because Harmenberg shows that in his invariant distribution, the level of consumption grows according to $\CLev_{t+1} = \PGro \CLev_{t}$.  Call $\mu^{\mRat}(\mRat)$ the proportion of permanent income accruing to persons at a given $\mRat$; then 
\begin{align}
  \log \cFunc(\mRat) \mu^{\mRat}(\mRat)\CLev_{t+1} - \log \cFunc(\mRat)\mu^{\mRat}\CLev_{t} &
\ = \log \PGro .
\end{align}
...
Thus in an economy that has settled into either a Harmenberg-invariant or a Szeidl-invariant distribution, growth rates of income and consumption measurable from microeconomic cross-section or panel data are the same.
