...
\begin{flalign}
  \text{(GPF)} && \PatPGro= {\Pat}/\PGro,  &&   \label{eq:GPFRaw}
\end{flalign}
...
%\begin{equation}
%  \begin{split}
    \begin{align}\label{eq:ValuePFAnalyticalAutarky}
...
%\begin{equation}
%  \begin{split}
    \begin{flalign} \label{eq:PFFVAC}
 &&     \overbrace{\DiscFac \PGro^{1-\CRRA} }^{\equiv \DiscAlt}  < 1  && \notag \\
\text{(PF-FVAC)}   &&  \DiscFac \Rfree \PGro^{-\CRRA}   < \Rfree/\PGro  &&
