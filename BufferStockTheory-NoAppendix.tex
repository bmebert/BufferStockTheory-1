...
% \date{\today}
\renewcommand{\forcedate}{November 23, 2021}\date{\forcedate}
  ...
  \parbox{0.9\textwidth}{
    \begin{center}
      \begin{tabbing}
        ...
        \texttt{~~~~~~PDF:~} \> \> \texttt{\href{https://\owner.github.io/BufferStockTheory/BufferStockTheory.pdf}{https://\owner.github.io/BufferStockTheory/LaTeX/BufferStockTheory.pdf}} \\ 
        \texttt{~~~Slides:~} \> \> \texttt{\href{https://\owner.github.io/BufferStockTheory/BufferStockTheory-Slides.pdf}{https://\owner.github.io/BufferStockTheory/LaTeX/BufferStockTheory-Slides.pdf}} \\
...
In the usual exposition, a dynamic budget constraint (DBC) combines several distinct events that jointly determine next period's $\mLevBF$ (given this period's choices); for the detailed analysis here, it will be useful to disarticulate and describe every separate step:\hypertarget{DBCParts}{}
...
\subsubsection{PF Unconstrained Solution Requires {\FHWC} and {\RIC}}\label{subsec:PFUncon}
...
\subsubsection{The Perfect Foresight Finite Value of Autarky Condition (\PFFVAC)}

We can define `autarky' as a circumstance under which capital markets do not exist, so the consumer simply spends all permanent noncapital income $\pLevBF$ in every period.  Because $\uFunc(xy) = x^{1-\CRRA}\uFunc(y)$, the value the consumer would achieve is
...
The first panel of Table~\ref{table:Required} summarizes: The PF-Unconstrained model has a nondegenerate limiting solution if we impose the {\RIC} and {\FHWC} (these conditions are necessary as well as sufficient).  Together the {\PFFVAC} and the {\FHWC} imply the {\RIC}.  If we impose the {\GIC} and the {\FHWC}, both the {\PFFVAC} and the {\RIC} follow, so {\GIC}+{\FHWC} are also sufficient.  But there are circumstances under which the {\RIC} and {\FHWC} can hold while the {\PFFVAC} fails (which we write \cncl{\PFFVAC}).  For example, if $\PGro=0$, the problem is a standard `cake-eating' problem with a nondegenerate solution under the {\RIC} (when the consumer has access to capital markets).% chktex 10
...
\subsubsection{`Individual Target Wealth' \texorpdfstring{$\mTrg$}{m}}\label{subsubsec:mTarget}
...
\begin{verbatimwrite}{\EqDir/TheoremMTargetIsStable}
  \begin{theorem}\label{thm:target}
    ...
    \begin{equation}
      ...
      \label{eq:mTarget}
    \end{equation}
    Moreover, $\mTrg$ is a point of `stability' in the sense that
...
\subsubsection{Individual Balanced-Growth `pseudo steady state' \texorpdfstring{$\mStE$}{m}}\label{subsubsec:mSteadyState}
...
The only difference between~\eqref{eq:balgrostable} and~\eqref{eq:mTargImplicit} is the substitution of $\Rnorm$ for $\bar{\Rnorm}$.\footnote{A third candidate `stable point' is the $\mGro$ where $\Ex_{t}[\log \mLev_{t+1}] = \log \PGro \mLev_{t}$; equivalently, $\Ex_{t}\left[\log\left((\mStE-\usual{\cFunc}(\mGro))\Rnorm+\pShk_{t+1}\tShk_{t+1}\right)\right]  = \log \mGro_{t}$.  Because the expectation of the log of a stochastic variable is less than the log of the expectation, this will be satisfied by an $\mGro < \mStE$.  For our purposes, little would be gained by an analysis of this point parallel to those of the other points of stability; but to accommodate potential uses,  the toolkit computes the value of this point as~\texttt{mLogTrg}.\label{thirdmeasure}}$^{,}$\footnote{Our choice to call to this the individual problem's `individual balanced-growth pseudo-steady-state' $\mStE$ is motivated by what happens in the case where all draws of all future shocks just happen to take on their expected value of 1.0.  (They unexpectedly always take on their expected values).  In that infinitely improbable case, the economy \textit{would} exhibit balanced growth.
  \begin{align*}
...
\begin{comment}
  ...
  with two associated Lemmas demonstrating that $\mStE < \Gro{\mRat}$ and $\mStE < \$\mTrg$.
\end{comment}

The proofs of the theorems are almost completely parallel; to save space, they are relegated to Appendix~\ref{sec:ApndxMTargetIsStable}.  In sum, they involve three steps:
\begin{enumerate}
...
\begin{comment}
  \begin{align*}
    \overbrace{\Grw_{\cRatAvg}^{n} \cRatAvg_{t}}^{\cRatAvg_{t+n}}(\Grw_{\cRatAvg} - 1)\PGro^{n} & =(\Grw_{\cv} - \PGro) \overbrace{\cv_{t}\Grw_{\cv}^{n}}^{\cv_{t+n}}
    \\
    (\Grw_{\cRatAvg}\PGro/\Grw_{\cv})^{n} \cRatAvg_{t}(\Grw_{\cRatAvg} - 1) & =(\Grw_{\cv} - \PGro) \cv_{t}
  \end{align*}
  and a corresponding equation will hold in $t+1$:
  \begin{align*}
    \\  \overbrace{\Grw_{\cRatAvg} \cRatAvg_{t}}^{\cRatAvg_{t+1}}(\Grw_{\cRatAvg} - 1)\PGro & =(\Grw_{\cv} - \PGro) \overbrace{\cv_{t}\Grw_{\cv}}^{\cv_{t+1}}
  \end{align*}
  \begin{align*}
    (\Grw_{\cRatAvg} - 1)\cRatAvg_{t}(\Grw_{\cRatAvg} - 1)\PGro & =(\Grw_{\cv} - \PGro) \cv_{t}(\Grw_{\cv} - 1)
  \end{align*}
  ...
  \end{align*}
  \begin{align}
    % \Grw_{\cRatAvg} - 1 & = \Grw_{\cv} - \PGro \\
    % \Grw_{\cRatAvg} + \pGro & = \Grw_{\cv}  \\
    \Grw_{\cv} & = \Grw_{\cRatAvg} + (\PGro - 1) \label{eq:aRatGroVscov}
  \end{align}
  \begin{align*}
